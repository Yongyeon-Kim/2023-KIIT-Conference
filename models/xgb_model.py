...
# 타겟 레이블을 이진 클래스로 매핑합니다.
y_train = np.where(y_train == 2, 1, 0)  # 원래 레이블이 1과 2인 경우
...
# 타겟 레이블을 이진 클래스로 매핑합니다.
y_valid = np.where(y_valid == 2, 1, 0)  # 원래 레이블이 1과 2인 경우
...
# 결과를 저장할 파일 열기
with open("models/log/XGB_acc.txt", "w") as file:
